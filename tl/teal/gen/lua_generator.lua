...
local lua_generator = { sandbox_options = {}, Options = {} }
...
function lua_generator.generate(ast, gen_target, opts)
   ...
   local emit_valid_thing_visitor_cbs = {
      after = function(_, node, _children)
         local out = { y = node.y, h = 0 }
         if opts.sandbox_options and opts.sandbox_options.banned_identifiers[node.tk] then
            add_string(out, "_not_allowed")
         else
            add_string(out, node.tk)
         end
         return out
   ...
   local function at_loop_point(out)
      if opts.sandbox_options then
         table.insert(out, #out, opts.sandbox_options.before_loop_end .. " ")
      end
   end

   local function at_loop_point_in_function(out)
      if opts.sandbox_options then
         table.insert(out, #out + 1, " " .. opts.sandbox_options.before_loop_end)
      end
   end
   ...
   visit_node.cbs = {
      ...
      ["local_declaration"] = {
         after = function(_, node, children)
            ...
            for i, var in ipairs(node.vars) do
               ...
               if opts.sandbox_options and opts.sandbox_options.banned_identifiers[var.tk] then
                  add_string(out, "_not_allowed")
               else
      ...
      ["op"] = {
         after = function(_, node, children)
            ...
            elseif node.op.op == ".." and opts.sandbox_options ~= nil then
               table.insert(out, opts.sandbox_options.concat_function_name)
               table.insert(out, "(")
               add_child(out, children[1])
               table.insert(out, ",")
      ...
      ["variable"] = emit_valid_thing_visitor_cbs,
      ["identifier"] = emit_valid_thing_visitor_cbs,
      ...
      ["argument"] = emit_valid_thing_visitor_cbs,
      ["type_identifier"] = emit_valid_thing_visitor_cbs,
